  ...
  if (!themeFromMenu) {
    chrome.storage.sync.get("colors", function (colorsList) {
      ...
      for (let id = 0; inputIDs[id]; id++) {
        ...
        document.getElementById(inputIDs[id]).style.color = colorIsLight(
          hexToRGB(theMe[id])
        );
      }
    });
    ...
    for (let color = 0; themeFromMenu[color]; color++) {
      document.getElementById(inputIDs[color]).value = themeFromMenu[
        color
      ].replace("#", "");
      document.getElementById(inputIDs[color]).style.backgroundColor =
        themeFromMenu[color];
      document.getElementById(inputIDs[id]).style.color = colorIsLight(
        hexToRGB(theMe[id])
      );
...
function clickHandler(e) {
  ...
  store([ourColor, theirColor, backgroundColor, ourTextColor, theirTextColor]);
}
